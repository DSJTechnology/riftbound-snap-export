...
async function loadImage(supabase: any, input: { training_image_id?: string; image_data?: string }): Promise<Uint8Array> {
  ...
  throw new Error('Either training_image_id or image_data required');
}
...
serve(async (req) => {
  ...
  try {
    ...
    const [bytes1, bytes2] = await Promise.all([
      loadImage(supabase, image1),
      loadImage(supabase, image2),
    ]);
